        ...
        <ol className="relative border-s border-gray-700">
          {workExperience.map((job, index) => (
            <li key={index} className="mb-10 ms-8">
              ...
              <div className="block mb-2 text-sm font-normal leading-none text-white">
                <time>{job.period}</time>
                <span className="ml-2">{job.location}</span>
              </div>
              {job.roles.map((role, roleIndex) => (
                <div key={roleIndex} className="mb-4">
                  <span className="bg-gray-700 text-white text-sm font-medium me-2 px-2.5 py-0.5 rounded ring-1 ring-gray-600 ms-3">
                    {role.title}
                  </span>
                  <ul className="list-disc pl-5 mt-2 text-base font-normal text-white">
